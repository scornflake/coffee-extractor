...
class Editor:
    def __init__(self, master, the_settings: Settings, *args, **kwargs):
    ...
    def create_video_canvas_view(self, master):
        ...
        width = self.movie.frame_width
        height = self.movie.frame_height

        self.video_canvas = tkinter.Canvas(master, width=width, height=height)
    ...
    def editing_controls(self, parent):
        ...
        area_ui = AreaEditingUI(master=frame, the_settings=self.settings, update_preview_callback=self.create_or_update_ui_overlay)
        area_ui.grid(row=0, column=0, sticky="nsew")

        # Now put in a flexible spacer (vertical)
        # frame.grid_rowconfigure(5, weight=1)

        tuning_ui = self.lcd_tuning_ui(frame)
        tuning_ui.grid(row=2, column=0, sticky="nsew")
    ...
    def lcd_tuning_ui(self, parent):
        ...
        tuning = TuningUI(master=frame, update_preview_callback=self.update_lcd_preview, the_settings=self.settings)
        tuning.grid(row=2, column=0, sticky="nsew")
        return frame
    ...
    def update_lcd_preview(self):
        ...
        opencv_image = extract_lcd_and_ready_for_tesseract(self.current_video_frame, self.video_frame_number, self.settings)
        # opencv_image = extract_lcd_and_ready_for_tesseract2(self.current_video_frame, self.video_frame_number, self.settings)
